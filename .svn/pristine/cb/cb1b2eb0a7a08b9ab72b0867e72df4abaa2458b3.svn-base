...
public class HelloWorld extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// After having add the file above that file should now appear in the
		// list
		response.getWriter().println("<html><head><title>SAP HANA Cloud Platform: MyDocs App</title></head><body>");
		CmisHelper cmisHelper = retrieveCmisHelperClass(request);
		List<MyDocsDTO> docs = cmisHelper.getDocumentsList();
	 ...
	 * 
	 * @param docs
	 * @return HTML string with the field for the file upload and the lkist of
	 *         all files
	 */
	private String buildHtmlPageForShowingAllDocuments(List<MyDocsDTO> docs) {
		String result = "";
		// Print the "Upload File" field prior to the list of files, so that the
		// end user can upload files
		...
		if (docs != null && docs.size() > 0) {
			for (int i = 0; i < docs.size(); i++) {
				...
				String row = "<li>" + "<a href=" + '"' + doc.getDownloadLink() + '"' + ">" + doc.getFilename() + "<a> (" + doc.getFileLength()
						+ " bytes)</li>";
				result += row;
			}
		}
