...
public class PersonServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		for (Person p : resultList) {
			out.write("<tr><td>"
					+ (p.getFirstName().length() > 0 ? XSSEncoder.encodeHTML(p
							.getFirstName()) : "&nbsp")
					+ "</td><td>"
					+ (p.getLastName().length() > 0 ? XSSEncoder.encodeHTML(p
							.getLastName()) : "&nbsp") + "</td><td>"
					+ p.getId() + "</td></tr>");
		}
		out.write("</table>");
...
//		personService.addPerson(request.getParameter("FirstName"),request.getParameter("LastName"));
	...
	protected void doGenerate(HttpServletRequest request, PrintWriter out)
			throws ServletException, IOException {
		Orgnazition org = new Orgnazition();
		for (int i = 0; i < 10; i++){
			org.setName("TestOrgnazition_00"+String.valueOf(i));
